...
- 5090 / H100 都行：8-GPU EP=8 + sequence parallel + bf16，expert FFN 挂 LoRA（r=8），每卡 ~5 GB。
...
KL_to_teacher **不进 reward**——它通过 slime 的 `--use-opd --opd-type sglang --opd-kl-coef β`
在 advantage 端作 shift。
...
| `router (per-layer)` | **LM loss（主信号）** | RoutingReplay 重放 indices（无梯度）但从当前 router **重算 gate weights**：`probs = scores.gather(1, replayed_idx)`，scores = softmax(F.linear(h, router.weight))。LM loss → expert_output × probs → probs → router.weight |
| `router (per-layer)` | **L_chunk_consistency（辅助，λ=0.05）** | adapter 单独算 router logits → softmax → chunk KL → router.weight |
| `expert LoRA A/B` | **LM loss** | LoRA delta 加在 expert FFN 输出上，LM loss 直接反传到 lora_A/lora_B |
| `SwitchHead.linear` | `L_switch_pg` | `−A_t · log π(switch_l)` 通过 σ_l 反传 |
| `SwitchHead.linear` (二次通道) | `L_barrier` | 超预算时 hinge² 通过 used 累计回到 σ_l (cost 是 `switch·n_new` 含 σ STE) |
...
> **Router 梯度不被 RoutingReplay 阻断**：RoutingReplay 锁定的是 expert indices（离散选择），但 gate weights（连续权重）由 `scores.gather(1, replayed_indices)` 从当前 router 重新计算。代码路径：`slime/utils/routing_replay.py:67-71` + `megatron/core/transformer/moe/moe_utils.py:719-720`。Router 学到的是"给定选中的 experts，分配多大比例的 gate weight"；更新后的 router 在下一轮 rollout 时用于 expert 选择。
...
| SwitchHead 梯度来源 | STE 通过 KL 一条 | PG（A_t · logπ） + barrier 两条 |
...
| `peft.LoraConfig` + `Mxfp4Config(dequantize=True)` | peft 不兼容 mcore 层；改用自定义 `LoRALinear` 包装 mcore ColumnParallelLinear |
| `freeze_non_controller` + 手工 `torch.distributed.all_reduce(p.grad)` | 改用 slime 标准 `--only-train-params-name-list`；Megatron 自管 grad allreduce |
...
5. **权重同步**：slime 自带 `update_weights_from_distributed` 把 Megatron 的 switch_head + expert LoRA + router 推到 SGLang student worker。
...
bf16 全程，expert FFN 挂 LoRA（r=8, α=16），router 解冻转 float32。**5090 没 nvlink**，所以：
...
`slime_adapter/` 当前 ~2500 LoC（含测试），单元测试 40/40 pass（11 controller core
+ 3 integration mock + 7 p_mix + 7 batched cache + 7 deepsets switch head + 7 lora）。
